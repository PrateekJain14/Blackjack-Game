...
def deal_cards(frame):
    # pop the next card from the top of deck and it to back of pack
...
def score_hand(hand):
    ...
    # Only one ace can have the value 11, which is reduced to 1 if busted
    score = 0
    ace = False
...
def deal_player():
    ...
    # # If Busted, check if there is an ace and subtract 10
...
# MAKING CARD FRAME TO HOLD CARD IMAGE
dealer_card_frame = tkinter.Frame(card_frame, background="green")
dealer_card_frame.grid(row=0, column=1, sticky='ew', rowspan=2)
...
# MAKING CARD FRAME TO HOLD CARD IMAGE - PLAYERS
player_card_frame = tkinter.Frame(card_frame, background="green")
player_card_frame.grid(row=2, column=1, sticky='ew', rowspan=2)
